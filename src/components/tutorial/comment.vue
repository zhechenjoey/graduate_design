<template>
  <div>
    <div class="commentArea" v-for="cmt in cmtlist" :key="cmt.id">
      <div>
        <div class="gd-flex" style="align-items:center;">
          ...
          <h3 v-cloak>{{ cmt.user_name }}</h3>
        </div>
        <span
          v-cloak
          v-if="cmt.cmted_id!=''"
          style="display:inline-block;min-width:25px;width:40px;"
        >&nbsp;回复</span>
        <label v-cloak v-if="cmt.cmted_id!==null">{{ cmt.cmted_name }}</label>
        <label v-cloak class="display">{{cmt.cmted_id}}</label>
        <pre
          style="width:450px;word-break:break-word;color:gray;padding-left:40px;"
          data-toggle="modal"
          data-target="#myModal"
          @click="commentInfo(item.tv_id,item.user_id,cmt.cmt_id,cmt.cmt_name)"
        >{{cmt.cmt_body}}</pre>
        <!-- 回复当前这一条回复 -->
        <i
    ...
    <div class="cmtarea" style="margin:30px 0;">
      <div class="first-cmt">
        <textarea style="height:100px;" class="gd-textarea input90" v-model="thisData.cmt_body"></textarea>
        <button type="button" class="btn blue" @click="sendComment">评论</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getComment() {
      const that = this;
      this.axios
        .post("/api/index.php/tutorial/getComment", {
          tv_id: that.$route.query.tv_id
        })
        .then(res => {
          if (res.data.code === 1) {
    ...
    sendComment() {
      ...
      var mydata = {
        ...
        tv_owner_id: this.data.tv_author,
        cmted_id: "",
        ...
        id: this.data.tv_id + this.$store.state.userid + new Date(),
        user_image: this.$store.state.userinfo.user_image
      };
      const that = this;
      this.axios
        .post("/api/index.php/tutorial/sendComment", mydata)
        .then(res => {
    ...
    deleteComment(id) {
      const that = this;
      this.axios
        .post("/api/index.php/tutorial/deleteComment", {
          id: id,
          tv_id: that.$route.query.tv_id
        })
        .then(res => {
          if (res.data.code === 1) {
            that.cmtlist.forEach((item, index) => {
              if (item.id === id) {
                that.cmtlist = that.cmtlist.splice(index, 1);
              }
            });
          }
        });
